...
// Use a CSS-only particle background on mobile, WebGL only on desktop
// This avoids the WebGL context loss issue entirely on mobile
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Create a fresh canvas each time — avoids context-loss from StrictMode
      ...
      try {
        renderer = new THREE.WebGLRenderer({
          canvas,
          alpha: true,
          antialias: false, // keep off for perf & compat
          powerPreference: "low-power",
        });
      } catch {
        // WebGL not supported — just show gradient background
        canvas.remove();
        return;
      }
      ...
      const allGeos: InstanceType<typeof THREE.BufferGeometry>[] = [];
  ...
  return (
    <section id="beranda" className="relative min-h-screen overflow-hidden flex items-center bg-[#0a1a0f]">
      ...
      {/* Deco blobs */}
      <div
        className="absolute -top-16 -left-16 w-64 sm:w-96 h-64 sm:h-96 rounded-full bg-green-500/20 blur-[80px] pointer-events-none"
        style={{ animation: "morphBlob 10s ease-in-out infinite" }}
      />
      <div
        className="absolute bottom-0 right-0 w-56 sm:w-80 h-56 sm:h-80 rounded-full bg-emerald-700/15 blur-[80px] pointer-events-none"
        style={{ animation: "morphBlob 14s 3s ease-in-out infinite" }}
      />

      {/* Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full pt-8 pb-16 lg:pt-16">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center min-h-[85vh]">

          {/* Left */}
          <div style={{ animation: "fadeUp 0.9s cubic-bezier(0.16,1,0.3,1) both" }}>
            {/* Badge */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold uppercase tracking-widest border border-green-400/30 bg-green-400/10 text-green-400 mb-5">
            ...
            {/* Heading */}
            <h1
              className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-black leading-[0.9] mb-5 text-white"
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center gap-2.5 bg-green-500 hover:bg-green-400 text-white font-semibold px-6 py-3.5 rounded-2xl transition-all hover:scale-105 hover:shadow-2xl hover:shadow-green-500/40 text-sm sm:text-base w-full sm:w-auto"
              >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 text-white/80 hover:text-white border border-white/20 hover:border-white/40 px-6 py-3.5 rounded-2xl transition-all font-medium text-sm sm:text-base w-full sm:w-auto"
              >
                Lihat Produk
          ...
          <div className="lg:hidden relative w-full" style={{ animation: "fadeUp 1s 0.3s cubic-bezier(0.16,1,0.3,1) both" }}>
            <div className="w-full h-56 sm:h-72 rounded-3xl overflow-hidden border border-white/10 shadow-2xl shadow-black/50 relative">
              <img
                src="https://images.unsplash.com/photo-1586201375761-83865001e31c?w=600&q=85&fit=crop"
                alt="Jamur Tiram Segar"
                className="w-full h-full object-cover block"
              ...
              <div className="absolute bottom-4 left-4 right-4 flex gap-3">
                <div className="flex-1 bg-black/60 backdrop-blur-sm rounded-xl p-3 text-center">
                  <div className="text-white font-bold text-base" style={{ fontFamily: "'Cormorant Garamond', serif" }}>500 kg</div>
                  <div className="text-white/60 text-[10px]">per bulan</div>
                </div>
                <div className="flex-1 bg-green-500/80 backdrop-blur-sm rounded-xl p-3 text-center">
                  <div className="text-white font-bold text-base" style={{ fontFamily: "'Cormorant Garamond', serif" }}>98%</div>
                  <div className="text-white/80 text-[10px]">kepuasan</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Scroll indicator */}
      <div className="absolute bottom-6 sm:bottom-9 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-40 text-white pointer-events-none">
        <div className="w-px h-8 sm:h-12 bg-gradient-to-b from-transparent to-current" style={{ animation: "fadeIn 1s 0.5s both" }} />
